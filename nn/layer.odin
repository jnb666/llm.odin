...
delete_layer :: proc{delete_linear, delete_encoder, delete_layernorm, delete_attention}
...
linear_forward :: proc{linear_forward_cpu, linear_forward_cuda}
...
linear_fwd_checkargs :: proc(l: ^Linear($D, $T), inp, out: Shape, loc := #caller_location) {
	assert(
		...
		"invalid input shapes", loc,
	)
}

linear_backward :: proc{linear_backward_cpu, linear_backward_cuda}
...
linear_backward_cuda :: proc(l: ^Linear(Cuda, $T), inp, dout, din: Array(Cuda, T), loc := #caller_location) where T == f32 || T == BF16 {
	...
	when USE_CUBLAS_DBIAS {
		...
		err = cublas.gemm(dtype, .OP_T, .OP_N, array.ptr(dout), array.ptr(inp), array.ptr(l.weight.grad), OC, C, BT, beta=1, 
							bias=array.ptr(dbias), dbias=have_bias)
		if err != nil {
			log.panic("no cublas matmul algoritm found:", err, location=loc)
		}
		if have_bias {
...
linear_bwd_checkargs :: proc(l: ^Linear($D, $T), inp, dout, din: Shape, loc := #caller_location) {
	assert(
		inp == din && din.dims[1] == l.weight.dims[1] && dout.dims[1] == l.weight.dims[0] && din.dims[0] == dout.dims[0] && (l.bias == nil || dout.dims[1] == l.bias.size),
		"invalid input shapes", loc,
	)
}
...
encoder_forward :: proc{encoder_forward_cpu, encoder_forward_cuda}

encoder_forward_cpu :: proc(l: ^Encoder(CPU, f32), inp: Array(CPU, i32), out: Array(CPU, f32),
							loc := #caller_location) #no_bounds_check {
...
encoder_backward :: proc{encoder_backward_cpu, encoder_backward_cuda}

encoder_backward_cpu :: proc(l: ^Encoder(CPU, f32), inp: Array(CPU, i32), dout: Array(CPU, f32), 
								loc := #caller_location) #no_bounds_check {
...
layernorm_forward :: proc{layernorm_forward_cpu, layernorm_forward_cuda}

layernorm_forward_cpu :: proc(l: ^Layernorm(CPU, f32), inp, out: Array(CPU, f32), train := true,
			loc := #caller_location) #no_bounds_check {
...
layernorm_fwd_checkargs :: proc(l: ^Layernorm($D, $T), inp, out: Shape, loc := #caller_location) {
	assert(
		inp.ndims >= 2 && inp.dims == out.dims && out.dims[out.ndims-1] == l.scale.dims[0],
		"invalid input shapes", loc,
	)
}

// backprop layernorm: accumulates gradient in din
layernorm_backward :: proc{layernorm_backward_cpu, layernorm_backward_cuda}
...
layernorm_backward_cuda :: proc(l: ^Layernorm(Cuda, BF16), inp, dout, din: Array(Cuda, BF16), loc := #caller_location) {
	...
	dbias := array.zeros_like(l.bias.grad)
	// exec and accumulate
	cudnn.execute(plan, {'i', 'y', 'x', 'w', 's', 'b', 'm', 'v'}, 
		{x, dy, array.ptr(dx), scale, array.ptr(dscale), array.ptr(dbias), mean, rstd})
	add(din, dx, din)
...
layernorm_bwd_checkargs :: proc(l: ^Layernorm($D, $T), inp, dout, din: Shape, loc := #caller_location) {
	assert(
		inp.ndims >= 2 && inp.dims == dout.dims && inp.dims == din.dims && inp.dims[inp.ndims-1] == l.scale.size,
		"invalid input shapes", loc,
	)
...
attention_forward :: proc{attention_forward_cpu, attention_forward_cuda}

attention_forward_cpu :: proc(l: ^Attention(CPU, f32), inp, out: Array(CPU, f32), train := true,
				loc := #caller_location) #no_bounds_check {
...
attention_fwd_checkargs :: proc(l: ^Attention($D, $T), inp, out: Shape, loc := #caller_location) {
	assert(
		inp.ndims == 3 && out.ndims == 3 && out.dims[2] == l.channels &&
		inp.dims[0] == out.dims[0] && inp.dims[1] == out.dims[1] && inp.dims[2] == l.channels*3, 
		"invalid input shapes", loc
	)
}

// backprop attention: overwrites gradient values in din
attention_backward :: proc{attention_backward_cpu, attention_backward_cuda}

attention_backward_cpu :: proc(l: ^Attention(CPU, f32), inp, dout, din: Array(CPU, f32),
			loc := #caller_location) #no_bounds_check {
...
attention_backward_cuda :: proc(l: ^Attention(Cuda, BF16), inp, dout, din: Array(Cuda, BF16), loc := #caller_location) {
	...
	one := f32(1)
	neg_inf := f32(-math.F32_MAX)
	cudnn.execute(plan, {'q', 'k', 'v', 'x', 'y', 'z', 'o', 'd', 's', 'c', 'n', '1'},
		 				{q, k, v, dq, dk, dv, l.out_ptr, doutput, stats, &scale, &neg_inf, &one})
}
...
attention_bwd_init_cuda :: proc(B, T, C, NH: int, loc := #caller_location) -> cudnn.BackendDescriptor {
	...
	defer cudnn.destroy(o, osum, osums, qk, qks, row, col, cmp, mask, val, p, p_t, 
		dout_v, dout_vs, delta, prod, prods, prods_t, k_t)
...
attention_bwd_checkargs :: proc(l: ^Attention($D, $T), inp, dout, din: Shape, loc := #caller_location) {
	assert(
		inp.ndims == 3 && dout.ndims == 3 && inp.dims == din.dims &&
		inp.dims[0] == dout.dims[0] && inp.dims[1] == dout.dims[1] && inp.dims[2] == l.channels*3, 
		"invalid input shapes", loc
	)
	when D == CPU {
		assert(l.att != nil, "intermediate activation not populated - run attention_forward in train mode", loc)
	} else {
		assert(!array.is_nil(l.stats) && l.out_ptr != nil, 
			"prior output and stats not populated - run attention_forward in train mode", loc)
	}
}
...
gelu_forward :: proc{gelu_forward_cpu, gelu_forward_cuda}
...
gelu_backward :: proc{gelu_backward_cpu, gelu_backward_cuda}
...
cross_entropy_loss :: proc{cross_entropy_loss_cpu, cross_entropy_loss_cuda}

cross_entropy_loss_cpu :: proc(logits: Array(CPU, f32), targets: Array(CPU, i32), losses: Array(CPU, f32), vocab_size: int, 
							   train := true, grad_accum_steps := 1, loc := #caller_location) #no_bounds_check {
...
cross_entropy_loss_cuda :: proc(logits: Array(Cuda, BF16), targets: Array(Cuda, i32), losses: Array(Cuda, f32), vocab_size: int, 
								train := true, grad_accum_steps := 1, loc := #caller_location) {
...
cross_entropy_checkargs :: proc(logits, targets, losses: Shape, vocab_size: int, loc := #caller_location) {
	assert(
		logits.ndims == 3 && targets.ndims == 2 && targets.dims == losses.dims && 
		logits.dims[0] == targets.dims[0] && logits.dims[1] == targets.dims[1] && logits.dims[2] >= vocab_size,
		"input shapes invalid", loc=loc
	)
}
...
tid :: proc(str: string) -> (value: u64) {
	digit_value :: proc(r: rune) -> u64 {
		ri := u64(r)
		switch r {
		case '0'..='9': return ri-'0'
		case 'a'..='z': return ri-'a'+10
		case 'A'..='Z': return ri-'A'+10
		}
		panic("invalid digit")
	}
